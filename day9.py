...
def part2_solve(input_lines: list[str], begin_range: int, end_range: int) -> int:
    ...
    for combo_index, combo in enumerate(sorted_combos):
        ...
        if begin_range >= expected_area >= end_range:
...
def main() -> None:
    ...
    # Guessed around and did preliminary runs until I found likely range and this works
    # Then brute force calculate in the range of where the largest area is,
    # only was able to do this by eliminating the top range by running it all night and getting no hits
